...
import { MainQuotesGrid } from './components/MainQuotesGrid';
...
import { TrendingUp, Activity, ShieldCheck, BarChart3, Landmark, Zap, Wrench, LineChart, Newspaper, ShoppingCart, Landmark as BankIcon, TrendingUp as InvestmentIcon } from 'lucide-react';
...
// ── Sección 4: Herramientas (clasificadas en 3 pestañas) ───────────────────────
// Tab 1: Consumo & Ahorro
    ...
    <>
      <Layout activeView={activeView} onViewChange={setActiveView}>
              ...
              <SectionReveal delay={80}>
                {!panicDismissed && (
                  <PanicBoard
                    ...
                    previousBluePrice={arbitrage?.dolares?.blue?.compra ?? 0}
                    onDismiss={() => setPanicDismissed(true)}
                  />
                )}

                {/* Grid Equilibrado de Cotizaciones Principales */}
                <div className="pt-6 pb-4">
                  <MainQuotesGrid
                    blue={arbitrage?.dolares?.blue}
                    mep={arbitrage?.dolares?.mep}
                    ccl={arbitrage?.dolares?.ccl}
                    crypto={rate ? { bid: rate.bid, ask: rate.ask } : undefined}
                    rateChange={rateChange}
                    loading={loading}
                  />

                  {/* Fila Equilibrada de 3 Indicadores Macroeconómicos */}
        ...
        ══════════════════════════════════════════════ */}
          {activeView === 'arbitrage' && (
            <div className="animate-fade-in">
              <SectionDivider
                ...
                description="Oportunidades de arbitraje en tiempo real entre tipos de cambio con guía paso a paso"
                badge="REAL-TIME"
                badgeColor="amber"
              />
              ...
              <RevealSuspense fallback={<SectionSkeleton cards={2} cols={2} height="h-96" />}>
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 pt-6 pb-4 items-start">
                  ...
                  {/* Columna Derecha: Calculadora + Spreads + Alertas + Plataformas */}
          ...
          {/* ═══════════════════════════════════════════════
            SECCIÓN 4: HERRAMIENTAS & CALCULADORAS (ORGANIZADO EN TABS)
        ══════════════════════════════════════════════ */}
          {activeView === 'herramientas' && (
            <div className="animate-fade-in">
              <SectionDivider
                ...
                description="Calculadoras financieras especializadas: consumo, créditos, inflación y rendimientos de arbitraje"
                badgeColor="violet"
              />

              {/* Sub-Tabs de Navegación */}
              <div className="flex flex-wrap gap-2 p-1.5 bg-white/5 rounded-2xl border border-white/10 my-6 max-w-xl">
                <button
                  onClick={() => setToolsTab('consumo')}
                ...
                >
                  ...
                  <span>Consumo & Ahorro</span>
                </button>
                <button
                  onClick={() => setToolsTab('creditos')}
                  className={`flex items-center gap-2 py-2 px-4 text-xs font-bold rounded-xl transition-all ${
                    toolsTab === 'creditos'
                      ? 'bg-accent-primary text-white shadow-md'
                      : 'text-slate-400 hover:text-white hover:bg-white/5'
                ...
                >
                  <BankIcon size={14} />
                  <span>Créditos & UVA</span>
                </button>
                <button
                  onClick={() => setToolsTab('inversion')}
                  className={`flex items-center gap-2 py-2 px-4 text-xs font-bold rounded-xl transition-all ${
                    toolsTab === 'inversion'
                      ? 'bg-accent-primary text-white shadow-md'
                      : 'text-slate-400 hover:text-white hover:bg-white/5'
                ...
                >
                  <InvestmentIcon size={14} />
                  <span>Inversión & Retornos</span>
                </button>
              </div>
              ...
              {/* Tab 2: Créditos & Deuda */}
              {toolsTab === 'creditos' && (
                <RevealSuspense fallback={<SectionSkeleton cards={2} cols={2} height="h-52" />}>
                  <div className="space-y-6 pb-4">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                      <UVALoanSimulator />
                      <LoanSimulator />
