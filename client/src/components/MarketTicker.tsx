...
export const MarketTicker: React.FC<MarketTickerProps> = ({ rate, dolares, global = [], merval = [] }) => {
    ...
    return (
        <div
            className="w-full bg-bg-app border-b border-border-subtle overflow-hidden relative"
            style={{ height: '36px' }}
            onMouseEnter={() => setPaused(true)}
            onMouseLeave={() => setPaused(false)}
        >
            {/* Left fade */}
            <div className="absolute left-0 top-0 bottom-0 w-12 z-10 pointer-events-none" style={{ background: 'linear-gradient(to right, var(--bg-app), transparent)' }} />
            {/* Right fade */}
            <div className="absolute right-0 top-0 bottom-0 w-12 z-10 pointer-events-none" style={{ background: 'linear-gradient(to left, var(--bg-app), transparent)' }} />

            {/* Paused indicator */}
            {paused && (
                <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 z-20 pointer-events-none">
                    <span className="flex items-center gap-1.5 text-[9px] font-bold text-slate-300 bg-bg-elevated border border-border-subtle px-3 py-1 rounded-full">
