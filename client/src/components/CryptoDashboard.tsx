...
export const CryptoDashboard: React.FC<{ cclRate?: number }> = ({ cclRate = 1200 }) => {
    ...
    const fetch = useCallback(async () => {
        setLoading(true);
        try {
            // Use CoinGecko free API (no key needed for top coins)
            const res = await window.fetch(
                'https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=bitcoin,ethereum,solana,cardano,ripple,polkadot,chainlink&order=market_cap_desc&per_page=7&page=1&sparkline=false&price_change_percentage=24h',
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {coins.map(coin => {
                        ...
                        return (
                            <div
                                key={coin.id}
                                className="bg-white/5 border border-white/10 rounded-xl p-3 hover:border-amber-400/20 hover:bg-amber-400/5 transition-all group"
                            >
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <div>
                                        <div className="text-[10px] font-bold text-white uppercase">{coin.symbol}</div>
                ...
                <table className="w-full text-xs">
                    <thead>
                        <tr className="text-slate-500 border-b border-white/5">
                            ...
                            <th className="text-right py-2 pr-2">ARS</th>
